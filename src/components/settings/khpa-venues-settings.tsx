...
export function KhpaVenuesSettings({ initialVenues, className }: Props) {
  ...
  const startEdit = (venue: KhpaVenue) => {
  ...
  const handleDelete = async (venue: KhpaVenue) => {
    const ok = await confirm({
      title: `刪除「${venue.name}」？`,
      description:
        "將一併刪除此場地所有對戰紀錄，且無法復原。若僅暫停使用，建議改按「停用」。",
      confirmLabel: "刪除",
      variant: "danger",
    });
    if (!ok) return;
    ...
    startTransition(async () => {
      try {
        await deleteKhpaVenue(venue.id);
